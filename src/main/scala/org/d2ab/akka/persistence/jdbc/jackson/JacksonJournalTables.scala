...
/*
  ...
  See: https://github.com/dnvriend/akka-persistence-jdbc/blob/e001c73cd3e7e7e7cbdd824692a0511436b15e6c/src/main/scala/akka/persistence/jdbc/journal/dao/JournalTables.scala
...
trait JacksonJournalTables {
  ...
  class Journal(tableTag: Tag) extends Table[JacksonJournalRow](tableTag, journalTableCfg.schemaName, journalTableCfg.tableName) {
    def * = (ordering, deleted, persistenceId, sequenceNumber, payload, identifier, manifest, writerUuid, sender, tags) <> (JacksonJournalRow.tupled, JacksonJournalRow.unapply)
    ...
    val identifier = column[Int](jacksonColumnNames.identifier)
    ...
    val sender = column[Option[Array[Byte]]](jacksonColumnNames.sender)
    val pk = primaryKey("journal_pk", (persistenceId, sequenceNumber))
    val orderingIdx = index("journal_ordering_idx", ordering, unique = true)
  }

  lazy val JacksonJournalTable = new TableQuery(tag => new Journal(tag))
}

case class JacksonColumnNames(payload: String = "payload", identifier: String = "identifier", manifest: String = "manifest",
                              writerUuid: String = "writer_uuid", sender: String = "sender")
